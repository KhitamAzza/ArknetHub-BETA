...
async function openDanaModal() {
  ...
  try {
    // FIX: POST instead of GET, action name matches GAS
    ...
    document.getElementById("danaId").value = data.id || "DEV-?????";
  } catch (err) {
...
async function submitPengajuanDana() {
  ...
  try {
    // FIX: action name matches GAS backend
    const res = await fetch(AUDIT_API_URL, {
      method: "POST",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify({
        ...
        petugas: currentOperator,
        ekstra: currentEkstra
      })
    });

    const data = await res.json();
    // FIX: GAS returns { success: true } not { status: "ok" }
    if (data.success) {
      showStatus("✓ Pengajuan berhasil dikirim", "ok");
    } else {
      showStatus(data.error || "Gagal mengirim", "error");
    }
  } catch (err) {
...
async function loadDanaHistory() {
  ...
  try {
    ...
    const res = await fetch(AUDIT_API_URL, {
      method: "POST",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify({ action: "getLog" })
    });
    const allData = await res.json();

    // FIX: GAS returns raw array, filter client-side by pembina name
    const list = Array.isArray(allData) ? allData : (allData.data || []);
    const filtered = list.filter(item => item.diajukanOleh === currentOperator);

    renderDanaHistory(filtered);
  } catch (err) {
